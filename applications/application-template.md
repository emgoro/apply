...
- A brief description of your project: Here is a video that we made for another grant that gives a great overview into our vision with this product https://drive.google.com/file/d/1OuNUippJbqXiGAKMWSjGXXhk-tfdMHfs/view?ts=67ecda12 
...
Possible integration Flow: Trigger POAP claim (if at qualifying event), log interaction with smart contract
...
With Leah's connections at SalesForce, Linkedin, and PayPal, we are getting confirmation on the integratation of our product at local conferences like Dreamforce. Mark is helping us establis the paperwork for our LLC, and draft up contracts between us and the conference hosts. In terms of documentation, we have done an in depth market analysis. Here are our results:
...
| -----: | ----------- | ------------- |
| 0a. | License | ConnectMag will operate under a Proprietary License, where the intellectual property rights remain with the company, but users will be granted usage rights for their customized QR code magnets. This will ensure that the product’s unique design, manufacturing process, and customization service are protected.|
| 0b. | Documentation | We will provide clear product documentation explaining the ordering process, customization options, and usage instructions for customers. The guide will also detail how users can link their QR codes to professional networking sites and integrate POAP for digital collectible rewards. Additionally, we will create a simple FAQ section to assist users with common inquiries.|
| 0c. | Testing and Testing Guide | While ConnectMag doesn't involve coding, we will implement a quality assurance process to test the functionality and durability of the QR code magnets. Testing will ensure the QR code's readability, magnet adhesion to various phone cases, and ease of removal without damaging the phone or case. The guide will explain how we validate these tests and measure the user experience. |
| 0d. | Article | We will publish an article detailing the development of ConnectMag, including the process of designing and manufacturing the custom QR code magnets, how they enhance networking experiences, and the possible partnership with POAP for digital collectible integration. The article will also explore the impact of using physical items like ConnectMag for professional networking at conferences, emphasizing its unique value proposition. |
| 1. | Custom QR Code Integration (Actual Creation)| We will create a feature that allows users to easily generate a unique QR code that links to their professional networking sites (e.g., LinkedIn, personal portfolio). Users will input their information via a web interface, and the system will automatically generate a custom QR code that is seamlessly integrated into the magnet. This feature will be linked with a simple, secure user authentication system, ensuring data privacy and protection. This means finalizing our manufactuer. |
| 2. | Magnet Customization Options | The second feature will enable users to customize the ConnectMag product by choosing different colors, designs, and sizes for their QR code magnets. This feature will allow users to select from a variety of pre-designed templates or upload their own design for the magnet. The customization options will be offered through an intuitive web interface, providing a user-friendly experience. Additionally, customers will be able to choose from a standard or premium magnet option based on their design preferences. |
| 3. | POAP Integration for Digital Collectibles |  for Digital Collectibles	The third feature will integrate Proof of Attendance Protocol (POAP) with the ConnectMag QR code, creating digital collectibles for event participants. When someone scans the ConnectMag QR code at a conference or event, they will receive a unique POAP token as a digital collectible that marks their attendance. This feature is designed to enhance networking and event participation by providing a tangible digital reward (POAP) for users, helping them celebrate their experience. |
